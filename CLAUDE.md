...
- **Not-yet-built (largest remaining pilot gaps):** maintenance-cost→ledger posting, receivable write-offs, reconciliation-exception resolution, statement PDF/CSV export, and the platform control plane (cross-org support/impersonation — large and sensitive). Real notification delivery workers and paid subscription changes are blocked on external provider config; don't invent provider identifiers.
