...
- **Read/write gate divergence (handled, follow-up noted):** the invite *command* gates on `resident.manage`, but the `user_relationships` read policy gates on `organization.manage` — so a `property_manager` (has `resident.manage`, not `organization.manage`) can invite but cannot read relationship rows, meaning the `/app/residents` directory shows every resident as `not_invited` for them. This is not a security issue (the command still enforces `resident.manage`). The invite button handles it gracefully: a `RELATIONSHIP_ALREADY_ACTIVE` response renders "Portal active" instead of an error. A complete fix — a `resident.manage`-scoped definer read RPC for invitation state, or relaxing the `user_relationships` read policy (a `13_...` matrix change) — is a deferred follow-up.
