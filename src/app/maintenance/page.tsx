...
export default async function ResidentMaintenancePage() {
  ...
  return <div className="min-h-screen bg-[#f6f8fb] pb-24">
    ...
    <nav aria-label="Resident" className="fixed inset-x-0 bottom-0 border-t bg-white/95 px-4 py-3 backdrop-blur sm:hidden"><div className="mx-auto flex max-w-md items-center justify-around text-xs font-medium"><Link className="text-muted-foreground" href="/home">Home</Link><Link className="text-muted-foreground" href="/payments/new">Payments</Link><Link className="text-primary" href="/maintenance">Maintenance</Link><Link className="text-muted-foreground" href="/messages">Messages</Link></div></nav>
  </div>;
}
