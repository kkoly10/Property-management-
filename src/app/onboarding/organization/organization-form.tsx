  ...
  return (
    <form action={action} className="space-y-6" noValidate>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="sm:col-span-2"><div className="flex items-center gap-1.5"><Label htmlFor="displayName">Workspace name</Label><InfoHint label="The name residents, owners, and staff see across Crecy. Use your operating or brand name — you can change it later." /></div><Input id="displayName" name="displayName" className="mt-2" placeholder="Northstar Property Group" required /><FieldError messages={state.fieldErrors?.displayName} /></div>
        <div className="sm:col-span-2"><Label htmlFor="slug">Workspace identifier</Label>{/* The shared primitive, not a hand-rolled copy of it: its `text-base sm:text-sm` is what keeps
        the control at 16px on a phone, and iOS zooms the whole page in on focus below that. This one
        field was the last input in the product still carrying a flat `text-sm`. */}<Input id="slug" name="slug" className="mt-2" placeholder="northstar" required /><p className="mt-2 text-sm text-muted-foreground">Identifies this Crecy workspace. It is not a public web address.</p><FieldError messages={state.fieldErrors?.slug} /></div>
        <div><div className="flex items-center gap-1.5"><Label htmlFor="customerPath">How you operate</Label><InfoHint label="Property manager: you manage units on behalf of other owners, so owner statements and management fees apply. Self-managing owner: you own the units you operate." /></div><NativeSelect id="customerPath" name="customerPath" className="mt-2" defaultValue="property_manager"><option value="property_manager">Property manager</option><option value="self_managing">Self-managing owner</option></NativeSelect><FieldError messages={state.fieldErrors?.customerPath} /></div>
        <div><div className="flex items-center gap-1.5"><Label htmlFor="headquartersCountryCode">Headquarters country</Label><InfoHint label="Sets your workspace default currency and address formatting. You can still add properties in other countries." /></div><NativeSelect id="headquartersCountryCode" name="headquartersCountryCode" className="mt-2" defaultValue="US"><option value="US">United States</option><option value="CA">Canada</option><option value="MX">Mexico</option></NativeSelect><FieldError messages={state.fieldErrors?.headquartersCountryCode} /></div>
        <div><Label htmlFor="defaultLocale">Default language</Label><NativeSelect id="defaultLocale" name="defaultLocale" className="mt-2" defaultValue="en-US"><option value="en-US">English (United States)</option><option value="es-MX">EspaÃ±ol (MÃ©xico)</option><option value="en-CA">English (Canada)</option><option value="fr-CA">FranÃ§ais (Canada)</option></NativeSelect><FieldError messages={state.fieldErrors?.defaultLocale} /></div>
      ...
      <div className="space-y-3">
        <label className="flex cursor-pointer items-start gap-3 rounded-lg border bg-muted/40 p-4 text-sm leading-5">
          ...
          <span data-testid="consent-statement">
            ...
            {consentDocuments.length ? " for this workspace." : "Crecy legal documents â none are available on this deployment."}
          </span>
        </label>
