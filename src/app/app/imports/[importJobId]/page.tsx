...
export default async function ImportDetailPage({ params }: { params: Promise<{ importJobId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {job.status === "completed" ? <Alert variant="info"><ShieldCheck className="h-5 w-5" /><AlertTitle>Import completed</AlertTitle><AlertDescription>{job.summary.commitResponse?.committed.properties ?? 0} properties and {job.summary.commitResponse?.committed.units ?? 0} units were committed. The report document is retained with this job.</AlertDescription></Alert> : null}
