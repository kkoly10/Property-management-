...
/** Each leg is named for what it does, so a job's own header is never another leg's label. */
const IMPORT_TYPE_HEADINGS: Record<string, string> = {
  portfolio: "Portfolio import",
  combined: "Combined portfolio import",
  leases: "Occupied lease import",
  residents: "Resident roster import",
  opening_balances: "Opening balance import",
};

/**
 * Report whatever the commit actually returned rather than what one leg happens to produce — a
 * combined commit creates properties, units, tenancies AND opening balances, and a summary hard-coded
 * to one leg's shape silently hides the rest.
 */
function summarizeCommitted(committed: Record<string, number> | null | undefined): string {
  const parts = [
    ["properties", "propert"],
    ["units", "unit"],
    ["tenancies", "tenanc"],
    ["openingBalances", "opening balance"],
    ["people", "person"],
    ["householdMembers", "household member"],
  ] as const;
  const written = parts
    .map(([key]) => [key, Number(committed?.[key] ?? 0)] as const)
    .filter(([, count]) => count > 0)
    .map(([key, count]) => {
      if (key === "properties") return `${count} ${count === 1 ? "property" : "properties"}`;
      if (key === "tenancies") return `${count} ${count === 1 ? "tenancy" : "tenancies"}`;
      if (key === "people") return `${count} ${count === 1 ? "person" : "people"}`;
      if (key === "openingBalances") return `${count} opening balance${count === 1 ? "" : "s"}`;
      if (key === "householdMembers") return `${count} household member${count === 1 ? "" : "s"}`;
      return `${count} unit${count === 1 ? "" : "s"}`;
    });
  return written.length ? `${written.join(", ")} were committed.` : "The batch committed with no new records.";
}
...
export default async function ImportDetailPage({ params }: { params: Promise<{ importJobId: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {job.status === "completed" ? <Alert variant="info"><ShieldCheck className="h-5 w-5" /><AlertTitle>Import completed</AlertTitle><AlertDescription>{summarizeCommitted(committed)} The report document is retained with this job.</AlertDescription></Alert> : null}
