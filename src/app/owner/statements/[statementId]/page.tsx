...
export default async function OwnerStatementDetailPage({ params }: { params: Promise<{ statementId: string }> }) {
  ...
  return <div className="min-h-screen bg-[#f6f8fb] pb-12 print:bg-white print:pb-0">
    <header className="border-b bg-white print:border-b-2"><div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-5 print:px-0"><Wordmark /><Badge variant="info" className="print:hidden">Crecy Owner</Badge></div></header>
    <main className="mx-auto max-w-5xl space-y-6 p-5 sm:py-8 print:p-0 print:pt-4">
      <Button asChild variant="ghost" size="sm" className="print:hidden"><Link href="/owner"><ArrowLeft className="h-4 w-4" />Owner workspace</Link></Button>
      ...
      {detail.mode === "error" || !item ? <Alert variant="destructive"><CircleAlert className="h-5 w-5" /><AlertTitle>Statement unavailable</AlertTitle><AlertDescription>Refresh and try again. Request {detail.requestId}.</AlertDescription></Alert> : <>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="success">Finalized · v{item.versionNumber}</Badge>{item.correctionReason ? <Badge variant="warning">Corrected statement</Badge> : null}</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.035em]">{item.snapshot.propertyName}</h1><p className="mt-2 text-sm text-muted-foreground">{item.snapshot.ownerName} · {new Date(`${item.snapshot.periodStart}T00:00:00`).toLocaleDateString()} – {new Date(`${item.snapshot.periodEnd}T00:00:00`).toLocaleDateString()}</p></div>{detail.mode === "ready" ? <div className="flex gap-2 print:hidden"><Button asChild variant="outline" size="sm"><a href={`/api/v1/owner-statements/${statementId}/export`}><Download className="h-4 w-4" />Download CSV</a></Button><PrintButton /></div> : null}</div>
