...
import { ArrowLeft, CircleAlert, Clock3, ShieldCheck } from "lucide-react";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Wordmark } from "@/components/brand/wordmark";
import { getResidentPaymentSessionOptions } from "@/lib/data/finance";
import { ResidentPaymentForm } from "./resident-payment-form";

export const dynamic = "force-dynamic";

export default async function NewResidentPaymentPage({ searchParams }: { searchParams: Promise<{ payment?: string }> }) {
  const [{ payment }, workspace] = await Promise.all([searchParams, getResidentPaymentSessionOptions()]);
  return <div className="min-h-screen bg-[#f6f8fb] pb-12">
    ...
    <main className="mx-auto max-w-3xl space-y-6 p-5 sm:py-8">
      ...
      <ResidentPaymentForm options={workspace.options} disabled={workspace.mode !== "ready"} />
    </main>
  </div>;
}
