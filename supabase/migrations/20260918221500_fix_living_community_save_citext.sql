...
-- The original function cast to unqualified ::public.citext; with search_path='' Postgres cannot resolve it.
...
begin
  ...
  if found then
    ...
    set
      subdomain=lower(trim(p_subdomain))::public.citext,
      display_name=trim(p_display_name),
      public_address_text=nullif(trim(coalesce(p_public_address_text,'')),''),
      headline=nullif(trim(coalesce(p_headline,'')),''),
      leasing_email=nullif(lower(trim(coalesce(p_leasing_email,''))),'')::public.citext,
    ...
    insert into public.living_community_profiles(
      ...
      p_property_id,v_organization_id,lower(trim(p_subdomain))::public.citext,trim(p_display_name),
      nullif(trim(coalesce(p_public_address_text,'')),''),
      nullif(trim(coalesce(p_headline,'')),''),
      nullif(lower(trim(coalesce(p_leasing_email,''))),'')::public.citext,
