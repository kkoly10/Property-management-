-- Phase 3: single-pass occupied-portfolio import (import_type='combined').
--
-- The portfolio leg creates empty properties/units; the occupied-lease leg activates a lease against a
-- unit that already exists. Real onboarding data is one spreadsheet where each row is "this unit, this
-- resident, this rent, this balance" — so requiring two passes forces the operator to split their file
-- and hand-reconcile it. This leg fuses both: one row creates (or reuses) the property and unit AND
-- activates the tenancy, so a portfolio goes from spreadsheet to operational in a single commit.
--
-- Postconditions per committed row: property + unit exist, household + primary member exist, lease is
-- active, tenancy is active/scheduled with a receivable account, a rent charge_schedule is armed (its
-- next run is generatable by generate_recurring_charges), and any opening balance posts a BALANCED
-- 1100/3900 journal. Unit-plan limits are enforced against the count of units this commit would add.
--
-- Forward-only; adds two commands and extends create_import_job to accept 'combined'. No new table or
-- RLS policy, so authority table/policy counts are unchanged.
begin;

-- ── create_import_job: accept the combined type (gated on the imports.full entitlement) ───────────
...
begin
  ...
  if p_import_type not in ('portfolio','leases','combined') then raise exception using errcode='0A000',message='IMPORT_TYPE_NOT_AVAILABLE'; end if;
...
create or replace function public.validate_combined_import(
...
declare
  ...
  v_normalized jsonb;
  v_property_name text; v_property_type text; v_address_line1 text; v_address_line2 text; v_locality text;
  v_subdivision_code text; v_postal_code text; v_country_code text; v_time_zone text;
  v_unit_code text; v_unit_type text; v_bedrooms_text text; v_bathrooms_text text; v_square_feet_text text;
  ...
  v_country_profile_id uuid; v_profile_currency char(3);
  v_operating_entity_id uuid; v_accounting_book_id uuid;
  v_existing_property_id uuid; v_existing_unit_id uuid;
  v_unit_key text;
...
begin
  ...
  if v_job.import_type<>'combined' then raise exception using errcode='23514',message='IMPORT_TYPE_MISMATCH'; end if;
  ...
  if not (p_mapping ?& array[
    'propertyName','propertyType','addressLine1','countryCode','timeZone','unitCode',
    'primaryFirstName','primaryLastName','leaseStartDate','rentAmountMinor','rentFrequency','currencyCode'
  ]) then
  ...
  for v_row in select * from private.import_rows r where r.import_job_id=p_import_job_id order by r.row_number loop
    v_errors := '[]'::jsonb;
    v_country_profile_id := null; v_profile_currency := null;
    v_operating_entity_id := null; v_accounting_book_id := null;
    v_existing_property_id := null; v_existing_unit_id := null;
    ...
    -- Computed unconditionally so an error row never inherits a prior row's dedupe key.
    ...
    if v_unit_code is null then v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','unitCode','code','REQUIRED','message','Unit code is required for a combined import.')); end if;
    ...
    if v_opening_text is not null and v_opening_text !~ '^-?[0-9]+$' then v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','openingBalanceMinor','code','INVALID_OPENING_BALANCE','message','Opening balance must be a whole number of minor units.')); end if;
    ...
    if v_start_date is not null and v_end_date is not null and v_end_date<v_start_date then
      v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','leaseEndDate','code','INVALID_LEASE_TERM','message','Lease end date cannot precede the start date.'));
    end if;
    ...
    if v_due_date is null and v_start_date is not null then v_due_date := v_start_date; end if;
    if v_rent_text ~ '^[0-9]+$' then v_rent_minor := v_rent_text::bigint; end if;
    if v_rent_minor is not null and v_rent_minor<=0 then
      v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','rentAmountMinor','code','INVALID_RENT_AMOUNT','message','Rent must be greater than zero.'));
    end if;
    if v_opening_text ~ '^-?[0-9]+$' then v_opening_minor := v_opening_text::bigint; end if;

    -- Country profile + the accounting book the row's money must land in.
    if v_country_code in ('US','CA','MX') then
      ...
      elsif v_currency_code is not null and v_currency_code<>v_profile_currency then
        v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','currencyCode','code','CURRENCY_MISMATCH','message','Row currency does not match the accounting book for this country.'));
      end if;
    end if;

    -- Reuse an existing property/unit when the file re-states one, so a combined import is safe to run
    -- against a partially onboarded portfolio instead of duplicating it.
    if jsonb_array_length(v_errors)=0 then
      ...
      if v_existing_property_id is not null then
        select u.id into v_existing_unit_id from public.units u
        where u.property_id=v_existing_property_id and lower(u.unit_code)=lower(v_unit_code);
      end if;

      -- Two rows for the same unit would activate overlapping tenancies. That is always an error, even
      -- in review mode, because the commit could not satisfy both.
      if exists(
        ...
        where earlier.import_job_id=p_import_job_id and earlier.row_number<v_row.row_number
          and earlier.normalized_data->>'unitKey'=v_unit_key
      ) then
        v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','unitCode','code','DUPLICATE_UNIT','message','This property and unit code appears more than once in the file; each unit can hold only one imported tenancy.'));
      end if;

      -- An already-occupied existing unit cannot take another overlapping tenancy.
      if v_existing_unit_id is not null and v_start_date is not null and exists(
        select 1 from public.tenancies t
        where t.unit_id=v_existing_unit_id and t.status in ('scheduled','active','notice_given','move_out_in_progress')
        ...
        v_errors := v_errors||jsonb_build_array(jsonb_build_object('row',v_row.row_number,'field','unitCode','code','TENANCY_OVERLAP','message','This unit already has a tenancy covering the imported lease term.'));
      end if;
    end if;

    v_normalized := jsonb_build_object(
      'unitKey',v_unit_key,
      'propertyName',v_property_name,'propertyType',v_property_type,'addressLine1',v_address_line1,'addressLine2',v_address_line2,
      'locality',v_locality,'subdivisionCode',v_subdivision_code,'postalCode',v_postal_code,'countryCode',v_country_code,'timeZone',v_time_zone,
      'unitCode',v_unit_code,'unitType',v_unit_type,'bedrooms',v_bedrooms_text,'bathrooms',v_bathrooms_text,'squareFeet',v_square_feet_text,
      'existingPropertyId',v_existing_property_id,'existingUnitId',v_existing_unit_id,
      'householdName',coalesce(nullif(trim(coalesce(v_household_name,'')),''),trim(coalesce(v_last_name,''))||' household'),
      ...
      'countryProfileId',v_country_profile_id,'operatingEntityId',v_operating_entity_id,'accountingBookId',v_accounting_book_id
    );
    ...
    if jsonb_array_length(v_errors)>0 then
      ...
      if v_existing_unit_id is null then v_new_unit_count := v_new_unit_count+1; end if;
      update private.import_rows r set normalized_data=v_normalized,validation_errors='[]'::jsonb,proposed_action='create_combined' where r.id=v_row.id;
    end if;
  end loop;
  ...
  v_totals := jsonb_build_object('rows',(select count(*) from private.import_rows r where r.import_job_id=p_import_job_id),
    'valid',v_create_count,'warnings',0,'errors',v_error_rows,'creates',v_create_count,'newUnits',v_new_unit_count,'updates',0,'skips',0);
  ...
  set status=v_status::public.import_status,mapping=p_mapping,validation_hash=case when v_status='ready' then v_validation_hash else null end,
      validated_at=now(),summary=j.summary||jsonb_build_object('totals',v_totals,'options',p_options)
  ...
  return jsonb_build_object('importJobId',p_import_job_id,'status',v_status,'totals',v_totals,
    'validationHash',case when v_status='ready' then v_validation_hash else null end,
    'errors',case when jsonb_array_length(v_all_errors)>50 then (select jsonb_agg(e) from (select e from jsonb_array_elements(v_all_errors) e limit 50) s) else v_all_errors end);
end;
$$;
revoke all on function public.validate_combined_import(uuid,jsonb,jsonb) from public,anon;
grant execute on function public.validate_combined_import(uuid,jsonb,jsonb) to authenticated;

-- ── commit_combined_import ────────────────────────────────────────────────────────────────────────
create or replace function public.commit_combined_import(
  p_import_job_id uuid,
...
declare
  ...
  v_row private.import_rows%rowtype;
  v_nd jsonb;
  v_plan_code text; v_unit_limit integer; v_existing_units integer; v_new_units integer;
  v_property_id uuid; v_unit_id uuid;
  ...
  v_properties_created integer := 0; v_units_created integer := 0;
...
begin
  ...
  begin
    ...
    -- Unit-plan limit, counted against the units this commit would actually ADD (rows that reuse an
    -- existing unit consume no new seat).
    select s.plan_code into v_plan_code from public.organization_subscriptions s
    ...
    where r.import_job_id=p_import_job_id and r.proposed_action='create_combined' and r.normalized_data->>'existingUnitId' is null;
    ...
    for v_row in select * from private.import_rows r where r.import_job_id=p_import_job_id and r.proposed_action='create_combined' order by r.row_number loop
      ...
      -- Property: reuse by identity, else create. Re-resolved here (not trusted from validation) so a
      -- property created by an EARLIER row of this same file is reused rather than duplicated.
      select p.id into v_property_id from public.properties p
      where p.organization_id=v_job.organization_id and lower(p.name)=lower(v_nd->>'propertyName')
        and lower(p.address_line1)=lower(v_nd->>'addressLine1')
        and lower(coalesce(p.locality,''))=lower(coalesce(v_nd->>'locality',''))
        and p.country_code=v_nd->>'countryCode'
      ...
      if v_property_id is null then
        insert into public.properties(
          ...
          v_job.organization_id,(v_nd->>'operatingEntityId')::uuid,(v_nd->>'accountingBookId')::uuid,(v_nd->>'countryProfileId')::uuid,
          v_nd->>'propertyName',v_nd->>'propertyType',v_nd->>'countryCode',v_nd->>'subdivisionCode',v_nd->>'locality',
          v_nd->>'postalCode',v_nd->>'addressLine1',v_nd->>'addressLine2',v_nd->>'timeZone','active',v_actor_id
        ) returning id into v_property_id;
        v_properties_created := v_properties_created+1;
      end if;

      select u.id into v_unit_id from public.units u
      where u.property_id=v_property_id and lower(u.unit_code)=lower(v_nd->>'unitCode');
      if v_unit_id is null then
        insert into public.units(organization_id,property_id,unit_code,unit_type,bedrooms,bathrooms,square_feet)
        values (
          v_job.organization_id,v_property_id,v_nd->>'unitCode',v_nd->>'unitType',
          nullif(v_nd->>'bedrooms','')::numeric,nullif(v_nd->>'bathrooms','')::numeric,nullif(v_nd->>'squareFeet','')::integer
        ) returning id into v_unit_id;
        v_units_created := v_units_created+1;
      end if;

      -- Re-check occupancy under the unit lock: validation ran earlier and a concurrent activation may
      -- have taken the unit since.
      perform 1 from public.units u where u.id=v_unit_id for update;
      if exists(
        select 1 from public.tenancies t
        where t.unit_id=v_unit_id and t.status in ('scheduled','active','notice_given','move_out_in_progress')
          and daterange(t.possession_start,coalesce(t.possession_end,'infinity'::date),'[]')
            && daterange(v_start_date,coalesce(v_end_date,'infinity'::date),'[]')
      ...
      insert into public.leases(
        ...
        v_lease_id,v_job.organization_id,v_property_id,v_unit_id,v_household_id,(v_nd->>'countryProfileId')::uuid,'imported',nullif(v_nd->>'externalReference',''),
      ...
      insert into public.tenancies(
        ...
        v_tenancy_id,v_job.organization_id,v_property_id,v_unit_id,v_household_id,v_lease_id,v_receivable_account_id,
      ...
      if v_opening_minor<>0 then
        ...
        insert into public.journal_transactions(
          ...
          'combined-import:'||p_import_job_id||':'||v_tenancy_id,v_currency,v_correlation_id,v_actor_id,jsonb_build_object('importJobId',p_import_job_id)
        ...
        if v_opening_minor>0 then
          ...
          values (v_journal_transaction_id,v_job.organization_id,(v_nd->>'accountingBookId')::uuid,v_ar_account_id,v_opening_minor,v_property_id,v_unit_id,v_tenancy_id,v_receivable_account_id,'Opening resident balance (import)');
          ...
          values (v_journal_transaction_id,v_job.organization_id,(v_nd->>'accountingBookId')::uuid,v_equity_account_id,v_opening_minor,v_property_id,v_unit_id,v_tenancy_id,v_receivable_account_id,'Opening resident balance offset (import)');
        else
          insert into public.journal_entries(journal_transaction_id,organization_id,accounting_book_id,ledger_account_id,debit_minor,property_id,unit_id,tenancy_id,receivable_account_id,memo)
          values (v_journal_transaction_id,v_job.organization_id,(v_nd->>'accountingBookId')::uuid,v_equity_account_id,abs(v_opening_minor),v_property_id,v_unit_id,v_tenancy_id,v_receivable_account_id,'Opening resident credit offset (import)');
          insert into public.journal_entries(journal_transaction_id,organization_id,accounting_book_id,ledger_account_id,credit_minor,property_id,unit_id,tenancy_id,receivable_account_id,memo)
          values (v_journal_transaction_id,v_job.organization_id,(v_nd->>'accountingBookId')::uuid,v_ar_account_id,abs(v_opening_minor),v_property_id,v_unit_id,v_tenancy_id,v_receivable_account_id,'Opening resident credit (import)');
        end if;
      ...
      insert into audit.audit_events(organization_id,actor_user_id,actor_type,action_code,resource_type,resource_id,correlation_id,after_data) values
        ...
        (v_job.organization_id,v_actor_id,'user','tenancy.activated','tenancy',v_tenancy_id,v_correlation_id,jsonb_build_object('householdId',v_household_id,'unitId',v_unit_id,'receivableAccountId',v_receivable_account_id)),
    ...
    values (v_report_document_id,v_job.organization_id,'import_report','Combined portfolio import report','system_generated','active',false,v_actor_id);
    if v_units_created>0 then
      ...
      values (v_job.organization_id,'active_units',v_existing_units+v_units_created,now(),'combined-import:'||p_import_job_id,'import_job',p_import_job_id);
    end if;

    v_response := jsonb_build_object('status','completed','committed',jsonb_build_object(
      'properties',v_properties_created,'units',v_units_created,'tenancies',v_tenancies_created,'openingBalances',v_opening_posted
    ),'reportDocumentId',v_report_document_id);
  ...
  exception when others then
    v_error := case
      when sqlerrm in ('TENANCY_OVERLAP','VALIDATION_HASH_CONFLICT','IMPORT_HAS_ERRORS','PLAN_LIMIT_EXCEEDED') then sqlerrm
...
$$;
revoke all on function public.commit_combined_import(uuid,text) from public,anon;
grant execute on function public.commit_combined_import(uuid,text) to authenticated;
