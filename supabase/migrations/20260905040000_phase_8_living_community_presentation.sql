...
-- A community hostname may reveal only the fields in the public profile RPC. It
-- never grants access to residents, leases, balances, documents, owner data,
-- vendor data, or internal operator notes.
begin;
...
create table public.living_community_profiles (
  ...
  public_address_text text check (
    public_address_text is null
    or length(trim(public_address_text)) between 1 and 300
  ),
  headline text check (
    headline is null
    or length(trim(headline)) between 1 and 160
  ),
  leasing_email citext check (
    leasing_email is null
    or length(trim(leasing_email::text)) between 3 and 254
  ),
  leasing_phone_e164 text check (
    leasing_phone_e164 is null
    or leasing_phone_e164 ~ '^\+[1-9][0-9]{7,14}$'
  ),
  ...
  public_notice_title text check (
    public_notice_title is null
    or length(trim(public_notice_title)) between 1 and 160
  ),
  public_notice_body text check (
    public_notice_body is null
    or length(trim(public_notice_body)) between 1 and 2000
  ),
  ...
  -- Presentation media must remain same-origin. This prevents an operator from
  -- turning a resident page into a third-party tracking pixel.
  check (
    hero_image_url is null
    or (
      length(hero_image_url) between 1 and 2048
      and hero_image_url ~ '^/media/[A-Za-z0-9][A-Za-z0-9/_-]*\.(webp|png|jpg|jpeg)$'
    )
  ),
  check (
    lobby_image_url is null
    or (
      length(lobby_image_url) between 1 and 2048
      and lobby_image_url ~ '^/media/[A-Za-z0-9][A-Za-z0-9/_-]*\.(webp|png|jpg|jpeg)$'
    )
  ),
  check (
    courtyard_image_url is null
    or (
      length(courtyard_image_url) between 1 and 2048
      and courtyard_image_url ~ '^/media/[A-Za-z0-9][A-Za-z0-9/_-]*\.(webp|png|jpg|jpeg)$'
    )
  ),
  check (
    model_home_image_url is null
    or (
      length(model_home_image_url) between 1 and 2048
      and model_home_image_url ~ '^/media/[A-Za-z0-9][A-Za-z0-9/_-]*\.(webp|png|jpg|jpeg)$'
    )
  )
);
...
-- The only anonymous contract. It returns publishable presentation fields only.
-- No internal organization/property ids are exposed and the hostname never acts
-- as an authorization grant.
create or replace function public.get_public_living_community_profile(
  p_subdomain text
)
returns jsonb
language sql
...
as $$
  select jsonb_build_object(
    'subdomain',cp.subdomain::text,
    'displayName',cp.display_name,
    'publicAddressText',cp.public_address_text,
    'headline',cp.headline,
    'leasingEmail',cp.leasing_email::text,
    'leasingPhoneE164',cp.leasing_phone_e164,
    'officeHours',to_jsonb(cp.office_hours_text),
    'amenities',to_jsonb(cp.amenities),
    'heroImageUrl',cp.hero_image_url,
    'lobbyImageUrl',cp.lobby_image_url,
    'courtyardImageUrl',cp.courtyard_image_url,
    'modelHomeImageUrl',cp.model_home_image_url,
    'publicNoticeTitle',cp.public_notice_title,
    'publicNoticeBody',cp.public_notice_body
  )
  from public.living_community_profiles cp
  where cp.subdomain::text = lower(trim(p_subdomain))
    and cp.status = 'published'
  limit 1;
$$;

revoke all on function public.get_public_living_community_profile(text)
  from public,anon,authenticated;
grant execute on function public.get_public_living_community_profile(text)
  to anon,authenticated;

-- Authenticated residents receive the same public-safe projection, plus the
-- tenancy id needed to match a presentation to one of their own homes.
  ...
  select jsonb_build_object(
    ...
    coalesce(
      jsonb_agg(
        ...
        order by display_name,tenancy_id
      ),
      '[]'::jsonb
    )
  )
...
revoke all on function public.get_resident_living_community_profiles()
  from public,anon,authenticated;
grant execute on function public.get_resident_living_community_profiles()
  to authenticated;
